  ...
  <div class="flex-1 flex flex-col p-4 pb-24 overflow-hidden">
    <div class="max-w-md w-full mx-auto flex-1 flex flex-col">
      ...
      <div class="mb-3 text-center">
        <div class="text-3xl font-light text-black mb-2">{rangeParts.book}</div>
        {#if rangeParts.sub}
          <div class="text-lg font-light text-gray-600">{rangeParts.sub}</div>
        {/if}
        {#if currentIndex === 0}
      ...
      {:else}
        <!-- Finished -->
        <div class="text-center">
          <div class="text-sm font-light text-gray-600 mb-3">Alle Verse in diesem Bereich wurden bearbeitet.</div>
          <button
            onclick={onShowNext}
